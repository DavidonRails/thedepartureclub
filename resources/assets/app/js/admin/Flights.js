...
angular.module('hoboAdmin.flights', [])
	.controller('FlightsController', function($scope, api, searchLocation, Alert) {
		var flights = this;
		flights.list = {
			data: [],
			pagination: []
		...
		flights.add = () => {
			$scope.$broadcast('flights:add');
		};

		flights.edit = (flight_id) => {
			$scope.$broadcast('flights:edit', flight_id);
		};

		$scope.$on('flights:refresh', () => {
			getFlights(undefined, {
				status: $scope.filter.status.key
			});
		});

		var getFlights = (paginate, filter) => {
			var url = 'admin/flights/';
			...
			api.call(url, filter)
				.success((res) => {
					if(res.status == 1) {
						flights.list.data = res.responseData.data;
		...
		$scope.paginate = (page) => {
			var url = flights.list.pagination[page];
			getFlights(url, {
				status: $scope.filter.status.key
			});
		};
		...
		flights.delete = function(flight_id){


			if(confirm('Delete flight?')){
				api.call('admin/flights/delete', {
					flight_id: flight_id
				})
					.success((r)=>{
	...
	.directive('flights', function($modal, api, Upload, Alert, searchAirports){
		...
		return {
			link: function($scope, $elem, args){
				...
				$scope.$on('flights:add', function(){
					$scope.errors = {};
					$scope.form = angular.copy(empty);
					$scope.method = 'insert';
					modal = $modal({
						scope: $scope,
						templateUrl: 'flights.modal.html',
						title: 'Add flight',
						show: false
					});
				...
				$scope.$on('flights:edit', function(e, flight_id){

					$scope.method = 'edit';
					$scope.errors = {};

					api.call('admin/flights/get/' + flight_id)
						.success((res) => {
				...
				$scope.action = (method, form) => {
					...
					if(typeof form.destination_airport_data != 'object'){
						Alert.info('Please select destination airport from autocomplete results');
						stop_submit = true;
					}
					...
					switch (method) {
						case 'insert':
							...
							api.call('admin/flights/add', data)
								.success((res) => {
									if(res.status == 1){
										if(form.aircraft_custom_image){
						...
						case 'edit':
							...
							api.call('admin/flights/edit', data)
								.success((res) => {
									if(res.status == 1){
										if(form.aircraft_custom_image){
											upload(form.aircraft_custom_image, form.aircraft_id, res.responseData.flight_id)
